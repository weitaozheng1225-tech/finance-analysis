...
INDICATORS: dict[str, dict] = {
    ...
    "us_30y_auction_btc": {
        ...
        "warn": 2.23, "crisis": 2.11, "direction": "below",
        "notes": (
            "美国 30Y 国债最新一次拍卖的 Bid-to-Cover 比率（投标总额 / "
            "中标总额）。阈值按 91 次拍卖历史分位校准（均值 2.33,σ 0.18）："
            "< 2.23 (p25) = 需求偏弱；< 2.11 (p10) = 需求严重不足，"
            "下次拍卖将面临更大压力。每月一次拍卖（通常月中），数据更新"
            "频率为月。数据源：Treasury Fiscal Data API（官方）。"
        ),
    },
    "us_30y_auction_indirect": {
        ...
        "group": "美债拍卖",
        "max_staleness_days": 50,
        "warn": 40.0, "crisis": 34.0, "direction": "below",
        "notes": (
            "美国 30Y 最新拍卖中间接投标人（含外国央行、主权基金）的中标份额。"
            "阈值按 87 次 30Y 拍卖历史分位校准（均值 47.8%,σ 12.1）："
            "< 40% (p25) = 海外需求边际下降；< 34% (p10) = 海外央行 / 外资"
            "明显退场，长端定价权回归国内买家与一级交易商，期限溢价容易"
            "快速走阔。注意：30Y 长债的间接投标份额历史上就比 10Y/Bills 低，"
            "此处用 30Y 自身分布作锚（之前误用跨期限的 [健康区间] 假设导致"
            "频繁误报 crisis）。对应 02_observation_indicators.md A5。"
        ),
    },
}
